...
- 생성된 `alt` 값은 빈 문자열이고, 비어 있으면 화면에서 `웨딩 사진 N`(순번)으로 채워진다.
  특정 사진에 진짜 설명을 넣고 싶을 때만 `gallery.json`의 해당 `alt`를 채우면 그 값이 우선한다.
  손으로 채운 값은 이 스크립트를 다시 돌리면 사라지므로, 순번으로 충분하면 비워두는 편이 안전하다.
...
카카오 개발자 콘솔의 도메인 설정이 **서로 다른 두 곳**에 있고, 하나만 해두면
증상이 헷갈린다. 실제로 한 번 겪었다.

| 설정 위치 | 무엇이 걸리나 |
|---|---|
| `[앱] > [JavaScript SDK 도메인]` | SDK 로드. 안 하면 카카오맵이 안 뜨고 공유 버튼도 죽는다 |
| `[앱] > [제품 링크 관리] > [웹 도메인]` | **공유 메시지의 링크.** 안 하면 카드는 가는데 눌러도 안 열린다 |

앞의 것만 등록했을 때의 증상: 지도는 정상, 공유 버튼도 눌리고, 채팅방에 카드와
메타정보까지 뜨는데, **이미지를 눌러도 반응이 없고 `청첩장 보기` 버튼이 아예
보이지 않는다.** 링크가 유효하지 않으면 카카오가 버튼을 렌더하지 않기 때문이다.
사이트 코드에는 아무 문제가 없으므로 여기를 먼저 볼 것.

둘 다 `https://changgi-suhyeon.github.io` 를 등록한다. 카카오 서버 쪽 설정이라
재배포는 필요 없고 즉시 반영된다.

참고: `noindex`는 스크랩을 막지 않는다. 색인 차단과 카카오톡 미리보기는 서로
간섭하지 않는 것을 실물로 확인했다(스크래퍼 UA로 HTTP 200, og 태그 4개 정상).
