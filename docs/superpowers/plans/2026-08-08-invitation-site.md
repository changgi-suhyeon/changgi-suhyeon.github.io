...
export default function RsvpForm({ closed, fallbackPhone }: Props) {
  ...
  // 재방문 안내는 있으면 좋은 것일 뿐이니 실패해도 조용히 넘어간다.
  useEffect(() => {
    try {
  ...
  // 그 찰나에 mealCount가 0으로 확정된다. 이어서 3을 입력해도 클램프는 위로 되돌리지
  // 않으므로 `참석 3명 · 식사 0명`이 남고, 서버는 0 <= 0 <= 3이라 정상 수락한다.
  // 조용히 식대가 한 명분 빠지는 경로다.
  useEffect(() => {
  ...
  async function submit(event: React.FormEvent) {
    ...
    try {
      ...
      // 오늘의 Worker는 저장이 확인된 경우에만 200을 주므로 이 분기는 도달하지 않지만,
      // 설계 문서가 최악으로 규정한 "실패를 성공으로 오인" 시나리오에 대한 클라이언트 쪽 방어선이다.
      if (response.ok && (data.ok !== true || typeof data.id !== 'number')) {
      ...
      // 저장은 성공했는데 setItem이 던지는 브라우저(쿠키·사이트 데이터 차단, 일부 인앱 웹뷰)에서
      // 아래 catch가 그것을 잡아 "전달에 실패했어요"를 띄우고, 하객이 재제출해 중복 행이 생긴다.
      // 식대가 한 명분 더 잡히는 경로다.
      setStatus('done')
      try {
      ...
      // Turnstile 토큰은 일회용이다. 성공 후에도 비우고 위젯을 리셋하지 않으면,
      // "수정이 필요하면 다시 제출" 경로에서 소비된 토큰이 재전송돼 서버가 403을 준다.
      window.turnstile?.reset(widgetIdRef.current)
      tokenRef.current = ''
    } catch {
      ...
      <div className="text-center text-sm leading-loose font-system">
        ...
        <button
          ...
          onClick={() => {
            ...
            // 돌아올 때 위젯을 다시 렌더해야 하는데, widgetIdRef를 비우지 않으면
            // 렌더 이펙트의 `if (widgetIdRef.current) return` 가드가 막아 위젯이 영영 안 뜬다.
            // 그러면 토큰이 발급되지 않아 수정 제출이 무한 실패한다 —
            // 인원을 정정하려는 하객이 새로고침 전까지 갇힌다.
  ...
  return (
    <span className="flex flex-col items-end gap-1">
      ...
      {/* 복사가 실패했을 때 하객이 계좌번호를 얻는 유일한 경로다.
          ...
          주석을 아래 `&& (` 안쪽으로 옮기지 말 것 — 그 자리는 단일 표현식만 받아
          주석과 엘리먼트가 형제가 되면 JSX 문법 오류다. */}
      {state === 'failed' && (
