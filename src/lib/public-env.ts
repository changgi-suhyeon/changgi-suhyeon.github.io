/**
 * RSVP 흐름이 의존하는 두 공개 환경변수의 게이트.
 *
 * 둘 다 deploy.yml의 `vars.*`에서 온다. 저장소 변수 이름을 잘못 쓰거나 아직 안 만들었으면
 ...
 *   PUBLIC_TURNSTILE_SITE_KEY === '' → turnstile.render 실패 → error-callback이 토큰을 비워
 *                                      서버가 전부 거부한다 → 아무도 참석 여부를 못 보낸다.
 *
 * 둘 다 하객만 겪고 신랑·신부는 끝까지 모른다. WEDDING_PRIVATE과 같은 원칙을 적용한다 —
 * 잘못된 배포보다 실패한 빌드가 낫다.
 *
